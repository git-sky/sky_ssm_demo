...
import cn.com.sky.mybatis.domain.Order;
...
public class Test2 {
	...
	public void testGetOrderById() {
		...
		/**
		 * 映射sql的标识字符串，
		 * cn.com.sky.mybatis.mapping.orderMapper是orderMapper.xml文件中mapper标签的namespace属性的值，
		 * getOrderById是select标签的id属性值，通过select标签的id属性值就可以找到要执行的SQL
		 */
		String statement = "cn.com.sky.mybatis.mapping.orderMapper.getOrderById";// 映射sql的标识字符串
	...
	public void testGetOrderById2() {
		 ...
		 */
		String statement = "cn.com.sky.mybatis.mapping.orderMapper.selectOrder";// 映射sql的标识字符串
		// 执行查询操作，将查询结果自动封装成Order对象返回
		Order order = sqlSession.selectOne(statement, 1);// 查询orders表中id为1的记录
		// 使用SqlSession执行完SQL之后需要关闭SqlSession
		sqlSession.close();
	...
	public void testGetOrderById3() {
		 ...
		 */
		String statement = "cn.com.sky.mybatis.mapping.orderMapper.selectOrderResultMap";// 映射sql的标识字符串
		// 执行查询操作，将查询结果自动封装成Order对象返回
		Order order = sqlSession.selectOne(statement, 1);// 查询orders表中id为1的记录
		// 使用SqlSession执行完SQL之后需要关闭SqlSession
		sqlSession.close();
